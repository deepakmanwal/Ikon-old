...
class Auth extends CI_Controller
{
	...
	function register()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			$this->form_validation->set_rules('birthday_month', 'Month', 'trim|required|xss_clean');
			$this->form_validation->set_rules('birthday', 'Day', 'trim|required|xss_clean');
			$this->form_validation->set_rules('birthday_year', 'Year', 'trim|required|xss_clean');

			//code for captcha is commented
			...
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($data = $this->tank_auth->create_user(
						...
						$this->form_validation->set_value('username')."@Ikon360.com",
						$this->form_validation->set_value('password'),
						$this->form_validation->set_value('birthday_year').'-'.$this->form_validation->set_value('birthday_month').'-'.$this->form_validation->set_value('birthday'),
					...
					if ($email_activation) {									// send "activate" email
						...
						unset($data['password']); // Clear password (just for any case)
						$this->session->set_flashdata('message', 'You have succcessfully registered. Login to continue.');
						redirect('auth/login');
	...
	function forgot_password()
	{
		...
		$this->load->model('tank_auth/users');
			$data = array();
			$data['questions'] = $this->users->get_security_questions();
	...
	function set_alternative_email()
	{
		$user_id = $this->input->get('user_id');
		$email = $this->input->get('alternative_email');
		$data = array('alternative_email'=>$email);
