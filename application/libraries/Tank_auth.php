...
class Tank_auth
{
	 ...
	 * @return	array
	 */
	function create_user($username, $first_name, $last_name, $email, $password, $birthday, $email_activation)
	{
	...
	{
		if (strlen($login) > 0) {
			if (!is_null($user = $this->ci->users->get_user_by_alternative_email($login))) {

				$data = array(
					'user_id'		=> $user->id,
					'username'		=> $user->username,
					'email'			=> $user->alternative_email,
